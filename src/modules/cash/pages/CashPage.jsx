...
function CashPage() {
  ...
  const onConfirmClose = async (data) => {
    try {
      ...
      // Imprime el ticket automáticamente al cerrar la caja
      try {
        await printCashClosing({ salesCount, systemTotal, paymentBreakdown, sales });
      } catch (printErr) {
        console.error("Error al imprimir cierre:", printErr);
        // La caja ya se cerró aunque falle la impresión
      }
      setToast({ message: "Caja cerrada e impresa correctamente", type: "success" });
